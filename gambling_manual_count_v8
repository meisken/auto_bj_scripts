...
const adjustedCombination = {
    ...
    "1" : {
        ...
        hasAceCombination : {
            "A2": {
                ...hasAceCombination["A2"],
                "5": actionTypes.double,
   
            },
            "A6": {
                ...hasAceCombination["A6"],
                "2": actionTypes.double,

            },
            "A8": {
                ...hasAceCombination["A8"],
                "5": actionTypes.doubleStand,
...
const handCheck = async (dealerCard, myCards, callbackOnResolve, isSecondHand) => {
    ...
    let adjustedHasAceCombination = hasAceCombination;
    let adjustedNormalCombination = normalCombination;
    
    if(roundedTrueCount !== 0){
        adjustedHasAceCombination = Object.assign(hasAceCombination,adjustedCombination[roundedTrueCount].hasAceCombination);
        adjustedNormalCombination = Object.assign(normalCombination,adjustedCombination[roundedTrueCount].normalCombination);
    }
    ...
    if(isPair && myCards.length < 3 && (callbackOnResolve === undefined || callbackOnResolve === null) && balanceCheck()) {
        ...
        adjustedHasAceCombination[`A${playerValueExcludeA}`][dealerNumber](dealerCard, myCards, callbackOnResolve, isSecondHand);
        ...
        adjustedNormalCombination[playerValue.toString()][dealerNumber](dealerCard, myCards, callbackOnResolve, isSecondHand)
...
const firstRun = () => {
    ...
    if(!(isValidNumber(parseInt(playedCardInput)) || parseInt(playedCardInput) === 0 || playedCardInput === "") ){
    ...
    if(!(isValidNumber(parseInt(multiplicationInput)) || parseInt(multiplicationInput) === 0 || multiplicationInput === "")){
        ...
        if(!multiplicationInput === "" && parseInt(multiplicationInput) != 0){
            cardCount.playedCard = parseInt(multiplicationInput) 
        } 
    }
...
const inertButton = () => {
    ...
    console.log("auto gambling manual_card_count_v8 inserted")
}
