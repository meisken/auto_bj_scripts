...
const fetchTrueCount = async () => {
    return new Promise(async (resolve, reject) => {
        try{
            ...
            cardCount.multiplication = err.multiplication
            cardCount.playedCard = err.playedCard
...
const buttonType = {
    hit: "[data-role='hit'] div.buttonContainer--e94e0",
    stand: "[data-role='stand'] > div.buttonContainer--e94e0",
    double: "[data-role='double'] > div.buttonContainer--e94e0",
    split: "[data-role='split'] > div.buttonContainer--e94e0",
    // bet: "div[data-state='enabled'].container--e0aac.container--dac87.enabled--b1cef.enabled--00dd8 div.gradientBorder--dfa99",
    bet: "div[data-state='enabled'] > div.border--63f59 > div.gradientBorder--51f83",
    no: "[data-role='no-position'] > div[data-role='no'] > div > div > div"
}
...
const resetCardCount = () => {
    ...
    console.log("reset card count", cardCount)
}
...
const yoloVision = () => {
    ...
    const renderBoxes = (canvasRef, boxes_data, scores_data, classes_data, ratios, triggeredCallback) => {
        ...
        for (let i = 0; i < scores_data.length; ++i) {
            ...
            if(parseInt(score) > customAiConfig.Threshold * 100){
...
const createCanvas = () => {
    const videoWrapper = document.querySelector("#video-wrapper > div ");
...
const visionAi = async () => {
    ...
    const waitVideoElement = () => {
        return new Promise((resolve,reject) => {
            const timer = setInterval(() => {
                ...
                const video = document.querySelector("#video-wrapper > div > div > div > video"); //#fmp4-video
                if(video) {
                    resolve(video);
                    clearInterval(timer);
                }
    ...
    const optimizedAnimationFrame = (callback) => {
        animationFrameTimers.push(
            setTimeout(() => {
                callback()
            },customAiConfig.framePerMilliseconds)
        )
    ...
    const detectFrame = async function() {
        ...
        const getDealerCardPoint = () => {
            const scoreElement = document.querySelector("div.dealerScore--6cc3b > div > div[data-role='score'].score--dc2e4");
        ...
        if(trueCount.playedCard > customAiConfig.maxPlayedCard ){
            resetCardCount()
        }
...
const betMoney = async () => {
    ...
    return new Promise(async (resolve,reject) => {
        ...
        let bettedTimes = 1;
        const { trueCount, playedCard } = getCurrentTrueCount();


        const matchedKey = (record, input) => {
...
const inertButton = () => {
    ...
    console.log("auto gambling anti_robot_v36_custom_14_test inserted")
}
