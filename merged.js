...
const visionAi = async () => {
    ...
    const publishable_key = "rf_Buyy3f7ERdP1htDovz1RykXxanX2";
    const toLoad = {
        model: "cardnumber",
    ...
    const loadRedCardModelPromise = new Promise(function(resolve, reject) {
        roboflow.auth({
            ...
            console.log("RedCardModel loaded")
            RedCardModel = m;
            //window.model = model;
            resolve();
        });
    ...
    const RedCardDetectFrame = () => {
        if(!RedCardModel) {
            console.log("fetch")
            return optimizedAnimationFrame(RedCardDetectFrame)
        };
        ...
        RedCardModel.detect(video).then(function(predictions) {
            if(predictions.length !== 0){
            ...
            console.log("CAUGHT", e);
            optimizedAnimationFrame(RedCardDetectFrame);
        });
    }
}    
...
const inertButton = () => {
    ...
    console.log("auto gambling V49_beta_wts inserted")
}
