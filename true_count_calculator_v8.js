...
const dataKey = "true-count-data"

const useLocalStorage = {
    save: (key, value) => {
        localStorage.setItem(key, JSON.stringify(value));
        console.info(`saved a value: ${localStorage.getItem(key)} with a key: ${key}`)
    },
    delete: (key) => {
        localStorage.removeItem(key);
        console.info(`deleted a column by a key: ${key}`)
    },
    get: (key) => {
...
const initData = () => {
    const loadData = useLocalStorage.get(dataKey);
    if(loadData){
        console.info(`loaded data:`, loadData)
        cardCount = loadData.cardCount,
        currentHands = loadData.currentHands;
    ...
    getCurrentTrueCount(false)
}

const getCurrentTrueCount = (localStorage = true) => {
    let current = {
        playedCard: 0,
        multiplication: 0
    };
...
const inertButton = () => {
    ...
    console.info("true_count_calculator_v8 inserted")
}
