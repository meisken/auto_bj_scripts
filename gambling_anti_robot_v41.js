...
const yoloVision = () => {
    ...
    const renderBoxes = (canvasRef, boxes_data, scores_data, classes_data, ratios, triggeredCallback) => {
        ...
        for (let i = 0; i < scores_data.length; ++i) {
            ...
            if(parseInt(score) > customAiConfig.Threshold * 100){
                ...
                if(
                    (
                        y1 > customAiConfig.detectableArea.y[0].min && y1 < customAiConfig.detectableArea.y[0].max &&
                        x1 > customAiConfig.detectableArea.x[0].min && x1 < customAiConfig.detectableArea.x[0].max &&
                        y2 > customAiConfig.detectableArea.y[1].min && y2 < customAiConfig.detectableArea.y[1].max &&
                        x2 > customAiConfig.detectableArea.x[1].min && x2 < customAiConfig.detectableArea.x[1].max
                    ) || customAiConfig.disableAreaCheck
                ){
                    redCardFoundCount++
                    if(redCardFoundCount >= customAiConfig.triggerTimeThreshold){
                        redCardFoundCount = 0;
    ...
    const detect = async (model, video, canvas, callback) => {
...
const visionAi = async () => {
    ...
    const detectFrame = async function() {
        ...
        const sendData = () => {
            ...
            if(customAiConfig.sendTrueCount){
...
const inertButton = () => {
    ...
    console.log("auto gambling anti_robot_v41 inserted")
}
...
const loadVisionModel = () => {
    ...
    return new Promise(async (resolve, reject) => {
        try{
            ...
            const remoteModelUrl = `https://cdn.statically.io/gh/meisken/cdn_script/main/vision_model/${customAiConfig.aiVersionName}/model.json`
...
(async function() {
    ...
    try{
        ...
        if(!aiConfig){
            ...
            console.warn("ai config is not defined")
        }
